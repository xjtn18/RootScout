...
bool TextField::handler(sf::Event& event, Program& p){
	...
	if (event.type == sf::Event::TextEntered){
		...
		case sf::Keyboard::Tab: // cycle through suggestions
			return true;
		...
		case sf::Keyboard::Return: // submit text to new/edited alarm
			try {
				std::string search = jb::lowercase(line.get_string());
				std::string filepath = p.trie.get_match(search);
				std::string command("vim --remote " + filepath);
				system(command.c_str()); // --remote tells MacVim to open this in the same window and tab
